...
function metricasDaEvolucao(student,cur,prev){
  const dc=derive(student,cur),dp=derive(student,prev);
  const bruto=[
    {rot:'% de gordura',   de:dp.fatPct,   para:dc.fatPct,   un:'%',  menorMelhor:true},
    {rot:'Massa magra',    de:dp.leanMass, para:dc.leanMass, un:'kg', menorMelhor:false},
    {rot:'Peso',           de:num(prev.weight), para:num(cur.weight), un:'kg', menorMelhor:null},
    {rot:'Cintura',        de:num(prev.circ_waist), para:num(cur.circ_waist), un:'cm', menorMelhor:true},
    {rot:'Massa muscular', de:num(prev.bio_muscle), para:num(cur.bio_muscle), un:'kg', menorMelhor:false},
    {rot:'Gordura visceral',de:num(prev.bio_visceral),para:num(cur.bio_visceral),un:'', menorMelhor:true},
  ];
  return bruto
    .filter(m=>m.de!=null&&m.para!=null&&!isNaN(m.de)&&!isNaN(m.para))
    .map(m=>{const d=+(m.para-m.de).toFixed(1);
      return {...m,delta:d,
...
function desenharEvolucao(ctx,{nome,de,ate,dias,metricas,marca,cref,arroba,logo}){
  const W=1080,H=1920;
  const serif='"Playfair Display",Georgia,serif';
  const sans='Inter,-apple-system,"Segoe UI",Roboto,sans-serif';
  const creme='#f5efe6', vinho='#7d1f35', vinhoC='#b8455f', ouro='#c9a227';

  const fundo=ctx.createLinearGradient(0,0,W*0.3,H);
  fundo.addColorStop(0,'#1a1315');fundo.addColorStop(0.5,'#120e10');fundo.addColorStop(1,'#1c1418');
  ctx.fillStyle=fundo;ctx.fillRect(0,0,W,H);
  const halo=ctx.createRadialGradient(W*0.5,H*0.26,0,W*0.5,H*0.26,W*0.9);
  halo.addColorStop(0,'rgba(125,31,53,.42)');halo.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=halo;ctx.fillRect(0,0,W,H);

  const centro=(txt,y,font,cor,esp)=>{
    ctx.font=font;ctx.fillStyle=cor;ctx.textAlign='center';
    if(!esp){ctx.fillText(txt,W/2,y);return;}
  ...
  if(dias>0)centro(dias===1?'1 dia de trabalho':dias+' dias de trabalho',
    486,'600 30px '+sans,'rgba(245,239,230,.42)');
  ...
  const areaTopo=580, areaBase=H-330;
  ...
  metricas.forEach((m,i)=>{
    ...
    ctx.fillStyle='rgba(245,239,230,.045)';cantos(80,y,W-160,alturaBloco-26,26);ctx.fill();
    ctx.strokeStyle='rgba(245,239,230,.10)';ctx.lineWidth=2;
    cantos(80,y,W-160,alturaBloco-26,26);ctx.stroke();

    ctx.textAlign='left';
    ctx.font='600 31px '+sans;ctx.fillStyle='rgba(245,239,230,.55)';
    ctx.fillText(m.rot.toUpperCase(),124,y+62);
    ctx.font='500 33px '+sans;ctx.fillStyle='rgba(245,239,230,.72)';
    ...
    ctx.font='800 84px '+sans;ctx.fillStyle=cor;
  ...
  if(logo){try{
    const s=104;ctx.save();cantos(W/2-s/2,yRodape-160,s,s,26);ctx.clip();
    ctx.drawImage(logo,W/2-s/2,yRodape-160,s,s);ctx.restore();
  }catch(e){}}
  ...
  ctx.fillStyle='rgba(125,31,53,.9)';ctx.fillRect(W/2-60,yRodape+86,120,4);
}
...
/* A janela do card: monta a imagem uma vez e oferece compartilhar ou baixar.
   ...
   consegue fazer ali. */
function CardEvolucao({student,evalData,prevEval,coach,onFechar}){
  const [url,setUrl]=useState(null);const [arquivo,setArquivo]=useState(null);
  const [erro,setErro]=useState(null);const [salvo,setSalvo]=useState(false);
  useEffect(()=>{let vivo=true;(async()=>{
    try{
      ...
      if(coach&&coach.logo_url){
        logo=await new Promise(r=>{const i=new Image();i.crossOrigin='anonymous';
          i.onload=()=>r(i);i.onerror=()=>r(null);i.src=coach.logo_url;});
      }
      const c=document.createElement('canvas');c.width=1080;c.height=1920;
      const dia=d=>new Date(d+'T00:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'short',year:'numeric'});
      desenharEvolucao(c.getContext('2d'),{
        nome:(student.name||'').split(' ')[0],
        de:dia(prevEval.date),ate:dia(evalData.date),
        ...
        arroba:coach&&coach.instagram,logo});
      const blob=await new Promise(r=>c.toBlob(r,'image/png'));
      if(!vivo||!blob)return;
      setUrl(URL.createObjectURL(blob));
      setArquivo(new File([blob],'evolucao.png',{type:'image/png'}));
    }catch(e){if(vivo)setErro('Não deu para montar a imagem neste aparelho.');}
  })();return()=>{vivo=false;};},[]);
  const baixar=()=>{if(!url)return;
    const a=document.createElement('a');a.href=url;a.download='evolucao-'+(student.name||'aluno').split(' ')[0].toLowerCase()+'.png';
    document.body.appendChild(a);a.click();a.remove();setSalvo(true);};
  const compartilhar=async()=>{
    try{
      if(arquivo&&navigator.canShare&&navigator.canShare({files:[arquivo]})){
        await navigator.share({files:[arquivo],title:'Evolução'});return;
      }
    }catch(e){if(e&&e.name==='AbortError')return;}
    baixar();
  };
  return(<div style={{position:'fixed',inset:0,zIndex:130,background:'rgba(10,8,10,.86)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,overflow:'auto'}} onClick={onFechar}>
    <div style={{maxWidth:330,width:'100%'}} onClick={e=>e.stopPropagation()}>
      {erro
        ? <div className="card" style={{lineHeight:1.5}}>{erro}</div>
        : url
          ? <img src={url} alt="Card de evolução" style={{width:'100%',borderRadius:16,display:'block'}}/>
          : <div className="card" style={{textAlign:'center',padding:'52px 0'}}><div className="spinner"/></div>}
      {!erro&&<button className="btn btn-primary" style={{width:'100%',marginTop:12}} disabled={!url} onClick={compartilhar}>
        Compartilhar</button>}
      {salvo&&<div className="s-meta" style={{marginTop:8,textAlign:'center',lineHeight:1.45,color:'var(--cream)'}}>
        Imagem salva. Manda no WhatsApp dela ou põe no story.</div>}
      <button className="btn btn-ghost" style={{width:'100%',marginTop:10}} onClick={onFechar}>Fechar</button>
    </div>
  </div>);
}
    ...
    <div>
      <div className="abar no-print">
        ...
        <div className="bgroup">
          ...
          {/* Só faz sentido com uma avaliação anterior: card de evolução sem
              antes é só uma tabela bonita. */}
          {prevEval&&<button className="btn btn-secondary" onClick={()=>setCardEvo(true)}>Card de evolução</button>}
          <button className="btn btn-primary" onClick={()=>window.print()}>Imprimir / PDF</button>
        </div>
      </div>
      {cardEvo&&prevEval&&<CardEvolucao student={student} evalData={evalData}
        prevEval={prevEval} coach={coach} onFechar={()=>setCardEvo(false)}/>}
            ...
            return(<div key={t.id} style={{marginBottom:16}}>
              ...
              {!complete&&ai<t.qtd_series&&<div>
                <div style={{display:'flex',gap:10,marginBottom:10}}>
                  ...
                  <div style={{flex:1}}><span className="lv-inlbl">Reps feitas</span><input className="lv-in" type="number" inputMode="numeric" placeholder="reps"
                ...
                <button className="lv-btn" onClick={()=>concluir(t,ai)}>
                  {faltaReps===t.id+'_'+ai?'✓ Concluir mesmo assim':'✓ Concluir série'}</button>
              </div>}
            </div>);})}
        </div>}
      </div>);})}
